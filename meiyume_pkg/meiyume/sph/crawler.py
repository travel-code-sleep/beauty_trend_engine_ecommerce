...
class Metadata(Sephora):
    ...
    def get_product_type_urls(self):
        ...
        df['scraped'] = 'N'
        df_clean = pd.DataFrame(sub_cat_urls, columns=['category_raw', 'product_type', 'url'])
        df_clean = df_clean[(df_clean.subcat_url.apply(lambda x: True if 'clean' in x else False))&(df_clean.product_type!='cleanser')]
